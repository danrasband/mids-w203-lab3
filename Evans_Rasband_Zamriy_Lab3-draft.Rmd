...
```{r}
crime[crime$west == 1 & crime$central == 1, "county"]
```

```{r}
county_check <-
  data.frame(t(rbind(round(mapply(mean, crime[crime$west == 1,]),4),
      round(mapply(mean, crime[crime$central == 1,]),4),
      round(mapply(mean, crime[crime$west == 0 & crime$central == 0,]),4),
      crime[crime$county == 71,])))
colnames(county_check) <- c("west", "central", "other", "selected")
county_check
```

```{r}
crime$exclude <- ifelse(crime$county %in% c(115, 71), 1, 0)
table(crime$exclude)
```
...
```{r}
wages <- c("wcon", "wtuc", "wtrd", "wfir", "wser_imp", "wmfg", "wfed", "wsta", "wloc")

crime$w_disparity <- 
  apply(crime[, wages], 1, FUN=max) - apply(crime[, wages], 1, FUN=min)
crime$w_disparity_tax <-crime$taxpc / crime$w_disparity 
crime$crmrte_violent <- crime$crmrte*crime$mix / (1+crime$mix)
crime$prbpris_v_crime <- crime$prbarr_imp*crime$prbconv*crime$prbpris
crime$crmrte_violent_shr <- crime$mix / (1+crime$mix)
crime$prbarr_imp_violent <- crime$crmrte_violent_shr / crime$prbarr_imp
```

```{r}
cor(crime$crmrte_violent_shr, crime$prbarr_imp_violent)
cor(crime$crmrte_violent_shr, crime$prbarr_imp)
```


```{r, fig.height = 4, fig.width = 5, fig.align="center"}
ggplot(crime, aes(crmrte_violent_shr, prbarr_imp_violent)) +
  geom_point() +
  facet_grid(region~.) +
  geom_smooth(method = "lm", se = FALSE)
```
...
crime$prbconv100_pc <- 100*crime$prbarr_imp * crime$prbconv
```

```{r}
model1a.ind_vars <- c("density", "west", "prbconv100_pc")
model1a.formula <- as.formula(paste("log(crmrte) ~",
                                   paste(model1a.ind_vars, collapse = " + "),
                                   sep = " "))
model1a <- lm(model1a.formula, data = crime[crime$exclude == 0,])
summary(model1a)
```
...
```{r}
model2a.ind_vars <- c("density", "west", "prbconv100_pc",
                     "polpc_imp.ln", "pctmin80", "west*polpc_imp.ln")
model2a.formula <- as.formula(paste("log(crmrte) ~ ",
                                   paste(model2a.ind_vars, collapse = " + "),
                                   sep = ""))
model2a <- lm(model2a.formula, data = crime[crime$exclude == 0,])
summary(model2a)
```
