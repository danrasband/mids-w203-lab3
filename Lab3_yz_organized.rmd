...
```{r}
ind_vars0 <- c("density", "west", "prbarr_imp", "prbconv")
crmrte_formula0 <- as.formula(paste("log(crmrte) ~", paste(ind_vars0, collapse = "+"), sep = ""))
crmrte_lm0 <- lm(crmrte_formula0, data = crime)
coeftest(crmrte_lm0)
summary(crmrte_lm0)$adj.r.squared
```


```{r}
ind_vars1 <- c("density", "prbarr_imp", "prbconv", "polpc_imp.ln",  "pctmin80")
...
```{r}
ind_vars2 <- c("density", "west","prbarr_imp", "prbconv", "polpc_imp.ln", "pctmin80",
               "west*polpc_imp.ln")
...
```{r}
par(mfrow=c(1,3))
plot(crmrte_lm0, which = 5)
plot(crmrte_lm2, which = 5)
plot(crmrte_lm_all, which = 5)
```

```{r}
check_vars <- c("region","density","prbarr_imp", "prbconv", "polpc", "wfed", "pctmin80")
crime[51, check_vars]
```

```{r}
mapply(summary,crime[,check_vars])
```

```{r}
par(mfrow=c(1,3))
hist(crmrte_lm0$residuals, breaks = 15)
hist(crmrte_lm2$residuals, breaks = 15)
hist(crmrte_lm_all$residuals, breaks = 15)
```
```{r}
se.crmrte_lm0 <- sqrt(diag(vcovHC(crmrte_lm0)))
se.crmrte_lm2 <- sqrt(diag(vcovHC(crmrte_lm2)))
se.crmrte_lm_all <- sqrt(diag(vcovHC(crmrte_lm_all)))
stargazer(crmrte_lm0, crmrte_lm2,crmrte_lm_all,
          type = "text", omit.stat = "f",
          se = list(se.crmrte_lm0, se.crmrte_lm2, se.crmrte_lm_all),
