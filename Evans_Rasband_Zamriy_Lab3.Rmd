...
- `density` (0.14): For each person per square mile increase in density, crime rate increases by 0.14% when everything else stays the same.
- `west` (-0.39): Crime rate in the West is 0.39% lower than in Central and Other regions on average (and controlling for all other included variables).
- `prbarr` (-1.69): For each point increase in probability of arrest, crime rate decreases by 1.69%.
- `prbconv` (-0.69): For each point increase in probability of arrest, crime rate decreases by 0.69%
...
cat('Correlation in the West:',
    cor(log(crime[crime$region=='west',]$crmrte),
        crime[crime$region=='west',]$polpc_imp.ln), "\n")
cat('Correlation in the Central:',
    cor(log(crime[crime$region=='central',]$crmrte),
        crime[crime$region=='central',]$polpc_imp.ln), "\n")
cat('Correlation in the Other:',
    cor(log(crime[crime$region=='other',]$crmrte),
        crime[crime$region=='other',]$polpc_imp.ln), "\n")
```

Now, to the actual model:

```{r}
model2.ind_vars <- c('density', 'west', 'prbarr_imp', 'prbconv',
...
- `density` (Before: 0.14, After: 0.09): The effect of density has decreased slightly. For each person per square mile increase in density, crime rate increases by 0.09%.
- `west` (Before: -0.39, After: -4.34): The effect of a county being in the West region has become much stronger, likely due to controlling for the effect of police per capita in the West. Crime rate is 4.34% lower in the West than in the rest of the state.
- `prbarr_imp` (Before: -1.69, After: -2.02): The probability of arrest has a slightly stronger effect. A single point of increase in the probability of arrest results in a 2.02% decrease in the crime rate.
- `prbconv_imp` (Before: -0.69, After: -0.74): The effect of the probability of conviction has also increased slightly. For each point increase in the probability of arrest, crime rate decreases by 0.74%.
- `polpc_imp.ln` (0.63): This coefficient indicates that a 1% increase in police per capita results in a 0.63% increase in crime rate.
- `pctmin80` (0.01): This coefficient indicates that a 1% increase in the minority population means a 0.01% increase in crime per capita.
- `west*polpc_imp.ln` (-0.63): A 1% decrease in police per capita in the West region results in a 0.63% decrease in crime rate for those counties.
...
The adjusted $R^2$ for this model is 80.1%, which is 12.9% higher than our first model:
...
```{r}
par(mfrow=c(1,2))
plot(model1, which = 1, caption = 'Model 1:\nResiduals vs. Fitted')
plot(model2, which = 1, caption = 'Model 2:\nResiduals vs. Fitted')
```
...
```{r}
par(mfrow=c(1,2))
plot(model1, which = 3, main = 'Model 1:\nScale-Location', caption = '')
plot(model2, which = 3, main = 'Model 2:\nScale-Location', caption = '')
```

```{r}
par(mfrow=c(1,2))
plot(model1, which = 5, main = 'Model 1:\nResiduals vs. Leverage',
     caption = '')
